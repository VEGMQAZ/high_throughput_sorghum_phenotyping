...
ss1 = dir(strcat('/home/vahid/Desktop/UsingLengthandUniqueAndOnlyNearestToBottom/Data/panicles.side.view/*.JPG'));
mkdir (pwd,'sideViewCheck')
...
try
    j
    IMG = imread(strcat('/home/vahid/Desktop/UsingLengthandUniqueAndOnlyNearestToBottom/Data/panicles.side.view/',ss1(j).name));
...
%     scatter(smoothPath(:,2),smoothPath(:,1),'*r','LineWidth',2);
    ...
    saveFileNameWidth = strcat(pwd,'/sideViewCheck/',ss1(j).name)
...
writetable(outputTable,'traitSide.xlsx');

widthDistTable=cell2table([result1 widthDist(:,1)]);
writetable(widthDistTable,'widthDistTableSide.xlsx')


widthDistTable=cell2table([result1 result5]);
writetable(widthDistTable,'widthDistTableIndexSide.xlsx')
...
writetable(resultMeanEach10Table,'lineByLineSide.xlsx','Sheet','MeanEach10')

resultMaxEach10Table=cell2table([result1 resultMaxEach10]);
writetable(resultMaxEach10Table,'lineByLineSide.xlsx','Sheet','MaxEach10')

resultMedianEach10Table=cell2table([result1 resultMedianEach10]);
writetable(resultMedianEach10Table,'lineByLineSide.xlsx','Sheet','MedianEach10')

resultMeanEach5Table=cell2table([result1 resultMeanEach5]);
writetable(resultMeanEach5Table,'lineByLineSide.xlsx','Sheet','MeanEach5')
resultMaxEach5Table=cell2table([result1 resultMaxEach5]);
writetable(resultMaxEach5Table,'lineByLineSide.xlsx','Sheet','MaxEach5')
resultMedianEach5Table=cell2table([result1 resultMedianEach5]);
writetable(resultMedianEach5Table,'lineByLineSide.xlsx','Sheet','MedianEach5')
